...
public class MCreatorNetherTemple extends Elementsvadersrandomadditions.ModElement {
	public MCreatorNetherTemple(Elementsvadersrandomadditions instance) {
		super(instance, 11);
	}
			...
			public boolean place(IWorld iworld, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if ((random.nextInt(1000000) + 1) <= 1000000) {
					...
					Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
							.getTemplateDefaulted(new ResourceLocation("vadersrandomadditions", "temple"));
					if (template == null)
						return false;
